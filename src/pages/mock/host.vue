<template>
    <cps-layout>
        ...
        <Modal
          ...
          @on-ok="handleAddHost"
          >
          <Form ref="modelView" :model="addModel" :rule="rule" :label-width="80">
            <Form-item label="项目名" prop="name">
              <Input v-model="addModel.name" placeholder="请输入项目名"></Input>
            </Form-item>
            ...
            <Form-item label="端口" prop="port">
              <Input v-model="addModel.port" placeholder="请输入端口"></Input>
            </Form-item>
          </Form>
        </Modal>
    </cps-layout>
...
export default {
  ...
  async mounted () {
    this.list = await this.$store.dispatch('common/host/QUERY_HOST')
    // console.log({list})
  },
  methods: {
    async handleAddHost () {
      this.$refs.$view.visible = true
      this.$refs.modelView.validate(state => {
        console.log({state})
      })
      // console.log(this.$refs.modelView)
      // const state = await this.$store('common/host/')
    },
    // ...mapActions([]),
    handleAdd () {
      this.$refs.$view.visible = true
